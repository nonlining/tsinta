...
size_t assignOneLineToInts(char *lineData, int **val, size_t numVal)
{
    ...
    /*numVal: check the # of value. it's 0 means it's the first time to read, and *val must be NULL*/
    /*out: # of int*/
    if (numVal == 0) {
        if (*val != NULL)
            free(*val);
        if((*val = (int*)revisedMalloc(sizeof(int) * MAXNUM, "assignOneLineToInts")) == NULL)
            return 0;
    }
    ...
    for (i = 0; lineData[i] != '\n' && lineData[i] != '\0'; ++i) {
        ...
        if (numVal == 0 && numInts >= MAXNUM) {
            if(revisedRealloc((void**)val, sizeof(int) * (numInts + 1), "assignOneLineToInts") == NULL)
                return 0;
        }
        ...
        sscanf(lineData + ((c[0] != '-') ? i : (i - 1)), "%d", &((*val)[numInts])); /*get int from lineData*/
    ...
    if (numVal == 0 && numInts < MAXNUM) {
        ...
        else if (revisedRealloc((void**)val, sizeof(int) * (numInts), "assignOneLineToInts") == NULL)
            return 0;
    }
...
Bool getOneLineFromFile(FILE *f, char **lineData)
{
    ...
    while ((c = getc(f)) != '\0' && c != '\n' && c != EOF) {
        ...
        if (i >= MAXNUM) {
            if(revisedRealloc((void**)lineData, sizeof(char) * (i + 1), "getOneLineFromFile") == NULL)
                return FALSE;
        }
    }
    (*lineData)[i] = '\0';
...
int** assignDataToVar(FILE *f, size_t *numData, size_t *numVal)
{
    ...
    int **data = (int**)revisedMalloc(sizeof(int*) * MAXNUM, "assignDataToVar");
    
    if (data == NULL)
        return NULL;

    char *lineData = (char*)revisedMalloc(sizeof(char) * MAXNUM, "assignDataToVar");
    ...
    *numData = *numVal = 0;
    while (getOneLineFromFile(f, &lineData)) {
        if (!allocEachData(&(data[*numData]), *numData, *numVal, &isNeedReuse)
            ...
            return NULL;
        if (*numData >= MAXNUM) {
            if(revisedRealloc((void**)data, sizeof(int*) * (*numData + 1), "assignDataToVar") == NULL)
                return 0;
        }
    ...
    if (*numData < MAXNUM) {
        ...
        else if(revisedRealloc((void**)data, sizeof(int*) * (*numData), "assignDataToVar") == NULL)
            return 0;
    }
    free(lineData);
    return data;
...
{*/
    ...
    for (i = 0; i < n; ++i)
        printf("%d , ", a[i]);
    free(a);*/
    /*char fileName[MAXNUM], *lineData;
    lineData = (char*)malloc(sizeof(char)*MAXNUM);
    scanf("%s", fileName);
    FILE *f = fopen(fileName, "r");
